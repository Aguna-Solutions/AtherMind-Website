    ...
    <section className="work" id="products" style={{ paddingTop: '64px', paddingBottom: '72px' }}>
      <div className="container">
        <header className="section-head text-center" style={{ marginBottom: '36px', textAlign: 'center' }}>
          ...
          <h2 className="section-title" style={{ fontSize: '2.7rem', fontWeight: 900, color: '#ffffff', marginBottom: '14px', letterSpacing: '-0.025em', lineHeight: 1.2 }}>
            One Platform to <span style={{ background: 'linear-gradient(135deg, #22d3ee 0%, #34d399 100%)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>Secure Every Database & Identity</span>
          </h2>
          <p className="section-sub" style={{ fontSize: '1.08rem', color: 'rgba(255, 255, 255, 0.82)', maxWidth: '760px', margin: '0 auto', lineHeight: 1.6 }}>
        ...
        <div className="control-plane-wrapper">
          ...
          <div className="cp-product-tabs" style={{ marginBottom: '24px' }}>
            <button
              className={`cp-tab-btn tab-dam ${activeTab === 'dam' ? 'active' : ''}`}
            ...
            <button
              className={`cp-tab-btn tab-pdm ${activeTab === 'pdm' ? 'active' : ''}`}
            ...
            <button
              className={`cp-tab-btn tab-aero ${activeTab === 'aero' ? 'active' : ''}`}
          ...
          {/* Main Control Plane Board with Dynamic Content for Active Product */}
          <div
            className="control-plane-board"
            style={{
              borderColor: activeData.accentColor,
              boxShadow: `0 30px 80px rgba(0,0,0,0.5), inset 0 0 30px ${activeData.accentColor}22`,
            }}
          >
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '16px', flexWrap: 'wrap', gap: '10px' }}>
              <span style={{ fontSize: '0.8rem', fontWeight: 800, textTransform: 'uppercase', letterSpacing: '0.08em', color: activeData.accentColor, background: `${activeData.accentColor}22`, padding: '4px 12px', borderRadius: '20px', border: `1px solid ${activeData.accentColor}44` }}>
            ...
            <div className="cp-flow-loop">
              <span>←</span>
              <div className="cp-loop-line"></div>
              <span className="cp-loop-title" style={{ color: activeData.accentColor }}>{activeData.topLoop}</span>
              <div className="cp-loop-line"></div>
              <span>→</span>
            </div>
            ...
            <div className="cp-columns-grid" style={{ marginTop: '24px', display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '16px' }}>
              {/* Column 1: Sources */}
              <div className="cp-column-card glass-panel" style={{ padding: '20px 16px', textAlign: 'center' }}>
                <h4 style={{ fontSize: '0.85rem', textTransform: 'uppercase', letterSpacing: '0.08em', opacity: 0.8, marginBottom: '20px', textAlign: 'center' }}>
                  {activeData.col1Title}
                </h4>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '16px', alignItems: 'center' }}>
                  {activeData.col1Items && activeData.col1Items.map((item, idx) => (
                    <div key={idx} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '6px' }}>
                      <div style={{ width: '42px', height: '42px', borderRadius: '50%', background: 'rgba(255,255,255,0.05)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '18px', border: `1px solid ${activeData.accentColor}44` }}>
                        {item.icon}
                      </div>
                      <span style={{ fontSize: '0.82rem', fontWeight: 600, color: 'var(--ink-1)' }}>{item.name}</span>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div className={`pillar-card glass-panel cp-pillar-card ${activeData.theme}`} style={{ padding: '20px 16px' }}>
                <div className="cp-radar-icon" style={{ width: '44px', height: '44px', margin: '0 auto 12px' }}>
                  {activeData.col2Icon}
                </div>
                <h3 style={{ fontSize: '1.1rem', textAlign: 'center', marginBottom: '16px' }}>{activeData.col2Title}</h3>
              ...
              <div className={`pillar-card glass-panel cp-pillar-card ${activeData.theme}`} style={{ padding: '20px 16px' }}>
                ...
                <h3 style={{ fontSize: '1.1rem', textAlign: 'center', marginBottom: '16px' }}>{activeData.col3Title}</h3>
                <ul className="cp-bullets-list" style={{ fontSize: '0.84rem', lineHeight: '1.5' }}>
              ...
              <div className={`pillar-card glass-panel cp-pillar-card ${activeData.theme}`} style={{ padding: '20px 16px' }}>
                ...
                <h3 style={{ fontSize: '1.1rem', textAlign: 'center', marginBottom: '16px' }}>{activeData.col4Title}</h3>
                <ul className="cp-bullets-list" style={{ fontSize: '0.84rem', lineHeight: '1.5' }}>
